...
//   await writeMcpImport(snapshot, 'tools.json');   // tools.json + tools.actions.mjs
//
...
//
// The mapping is the inverse of json-schema.mjs:
...
import { canonical, object, own, finite, requireAt } from './common.mjs';
import { mcpListTools } from './mcp-client.mjs';
...
// Writes <jsonPath> and, beside it, <name>.actions.mjs.
...
export async function writeMcpImport(snapshot, jsonPath) {
  ...
  const modulePath = join(dirname(jsonPath), `${name}.actions.mjs`);
