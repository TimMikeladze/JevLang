...
export interface Journal {
  beginStep(key: string, target: string, now: number): Promise<'new' | 'running' | StepResult> | 'new' | 'running' | StepResult;
  ...
  takeDue(now: number): Promise<[string, JSONValue][]> | [string, JSONValue][];
