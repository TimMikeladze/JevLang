...
import { handleMessage, makeMcpConn, modernVersion, metaProtocolVersion, metaClientCapabilities, inputRequired, McpError } from '../src/mcp.js';
import { mcpConnect, mcpClose } from '../src/mcp-client.js';
...
test('the proxy gates another server: allow forwards, deny blocks, ask asks a person', async t => {
  if (skipUnlessInMonorepo(t)) return;
  const upstreamPath = fileURLToPath(new URL('../examples/mcp-policy-server.js', import.meta.url));
