...
//   UPDATE_README=1 bun test test/readme-examples.test.js
...
import { policy as ticketRouter } from '../examples/ticket-router.js';
...
function scratchProject() {
  ...
  symlinkSync(join(root, 'src', 'cli.js'), join(dir, 'node_modules', '.bin', 'jev'));
  // Plain `.js` needs a nearby `"type": "module"` to run the examples' `import`s.
  writeFileSync(join(dir, 'package.json'), JSON.stringify({ name: 'scratch', private: true, type: 'module' }));
...
test('README captured runs match what the examples print', async () => {
  ...
  try {
    ...
    const support = await import(pathToFileURL(join(dir, 'support.js')).href);
