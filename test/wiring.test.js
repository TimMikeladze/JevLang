...
import { jevWiring, startWiring, wiringProblems } from '../src/wiring.js';
import { webhookSource, caseEvent, outcomeEvent, standardWebhooksVerifier, standardWebhooksSign, envSecret, githubVerifier } from '../src/webhooks.js';
import { harvestFind, harvestStatus } from '../src/harvest.js';
...
test('a shadow policy runs beside the primary and says what it would have done', async () => {
  ...
  const shadow = (await import('../examples/ticket-router.js')).policy;
  const changed = (await import('../src/index.js')).compile({
    ...shadow.toJSON(),
