...
report.widths = await page.evaluate(() => ({
  ...
  demoMin: Math.min(...[...document.querySelectorAll('.section:not(#start) .demo > .frame, .section:not(#start) .demo > .diagram')].map((f) => Math.round(f.getBoundingClientRect().width))),
}));
...
const policyBody = readFileSync(new URL('../README.md', import.meta.url), 'utf8').match(/```js file=policy\.mjs\n([\s\S]*?)```/)[1];
await page.click('#quick-start .frame:has(.name:text-is("policy.mjs")) [data-copy-code]');
...
expect(report.copy.code && report.copy.install === 'npm install jevlang', 'copy buttons copy the shown text');
...
report.anchors = [];
for (const id of [...sections.map((s) => s.id), 'boundaries', 'start']) {
