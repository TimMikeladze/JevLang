# JevLang — The model answers, your policy decides

jevlang is an open source policy engine for decisions an LLM used to make inside a prompt: routing, triage, approvals, guarding an agent's tools. You declare the questions the model answers and the rules that act on them, in plain TypeScript or Python (https://github.com/TimMikeladze/JevLang). Made by linesofcode (https://x.com/linesofcode).

Install: `npm install jevlang` · Currently v0.1.0
...
Run npm install jevlang, save the file as policy.mjs, and run it with node policy.mjs. It needs no model, account or network: policy.decide() reads the answers you hand it and returns the action.
...
Save the policy as gate.mjs and run it
...
```sh
$ echo '{"hook_event_name":"PreToolUse","tool_name":"Read","tool_input":{"file_path":"README.md"}}' | npx jev gate hook gate.json gate-options.json
...
```sh
$ echo '{"hook_event_name":"PreToolUse","tool_name":"WebFetch","tool_input":{"url":"https://example.com"}}' | npx jev gate hook gate.json gate-options.json
...
```sh
$ echo '{"hook_event_name":"PreToolUse","tool_name":"Bash","tool_input":{"command":"rm -rf build"}}' | npx jev gate hook gate.json gate-options.json
...
Save this as serve.mjs and start it
...
Jev Cloud runs this engine for many tenants over HTTP at cloud.jevlang.sh (https://cloud.jevlang.sh): deploy, promote, evaluate and replay. Every route but /healthz takes Authorization: Bearer jev_live_…, and the organization comes from the credential, never the path. Deployments never change once published; promote with expect is the only thing that moves production.
...
Proven, judged and missing, in that order — the same honesty the engine ships with. The full evidence table is in COMPATIBILITY.md (https://github.com/TimMikeladze/JevLang/blob/main/COMPATIBILITY.md).
...
- bun test here: 101 pass, 25 skip, 0 fail, offline. The skips are the differential tests that need the Racket monorepo beside this package.
