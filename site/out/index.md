...
Jev Cloud runs this engine for many tenants over HTTP at cloud.jevlang.sh (https://cloud.jevlang.sh): deploy, promote, evaluate. Every route takes Authorization: Bearer jev_live_…, and the organization comes from the credential, never the path. Deployments never change once published; promote with expect is the only thing that moves production.
